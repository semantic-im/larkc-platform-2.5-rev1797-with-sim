...
import eu.larkc.core.pluginManager.remote.Servlet.Tomcat.OperationDescriptor;
 ...
 * 
 */
public class PluginServlet extends LarkcServlet {
	...
	protected void invokePlugin(Plugin plugin, UrlParameters parameters,
		...
		// SPARQLQueryImpl(SampleQueries.WHO_KNOWS_TIM_BERNERS_LEE).toRDF());
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// Attempt to set a system property here that ORDI understands =>
		// storage-folder
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			String message = "Unable to instantiate plug-in due to: "
					+ e.getMessage();
			log(message);
			throw new IOException(message, e);
		}
	...
	protected void doPut(HttpServletRequest request,
		...
		if (plugin == null) {
			response.sendError(ERROR_PLUGIN_NOT_INITIALISED,
					"ERROR: No plug-in with id '" + requestUrl + "' found.");
		} else {
			...
			String operation = parameters
					.getValue(OperationDescriptor.OPERATION);

			if (operation == null) {
				response.sendError(ERROR_INVALID_ARGUMENTS,
						"ERROR: No operation specified for invoking '"
								+ requestUrl + "'.");
			} else if (operation.equals(OperationDescriptor.OP_INVOKE)) {
				try {
					invokePlugin(plugin, parameters, input, output);
				} catch (RuntimeException e) {
					String message = "Plugin invocation failed: "
							+ e.getMessage();
					log(message);
					throw e;
				}
	...
	protected Plugin instantiatePlugin() throws IOException {
		...
		if (classname == null)
			throw new IOException("Config error: No '" + PLUGIN_CLASSNAME
					+ "' initialisation parameter in web.xml");
		...
		try {
			...
			String message = "Config error: Unable to locate plug-in class '"
					+ classname + "'";
			log(message);
			throw new IOException(message, e);
		}
		...
		if (pluginProperties != null) {
			...
			try {
				Constructor<?> constructor = pluginClassObject
						.getConstructor(Properties.class);
				log("Instantiating plug-in with constructor: (Properties)");
				return (Plugin) constructor.newInstance(pluginProperties);
			} catch (Exception e) {
				String message = "Instantiation with constructor( Properties ) failed with "
						+ e;
				log(message);
			}
		}

		// No properties or no constructor that takes properties, so use the
		// default constructor
		try {
			...
			String message = "Instantiation with default constructor failed with "
					+ e;
			log(message);
		}

		throw new IOException("Unable to instantiate the plug-in '" + classname
				+ "'. No suitable constructor found or accessible.");
	}
	...
	private Map<String, Plugin> mPluginInstances = Collections
			.synchronizedMap(new HashMap<String, Plugin>());
