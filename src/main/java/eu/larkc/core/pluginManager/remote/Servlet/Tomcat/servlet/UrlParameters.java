...
public class UrlParameters {
	...
	public UrlParameters(Map rawParameterMap) {
		...
		for (Map.Entry<String, String[]> entry : (Set<Map.Entry<String, String[]>>) rawParameterMap
				.entrySet()) {
			...
			for (int i = 0; i < encodedValues.length; ++i) {
				try {
					decodedValues[i] = URLDecoder.decode(encodedValues[i],
							"utf-8");
	 ...
	 * @return first value mapped to <code>key</code> or <code>null</code> if no
